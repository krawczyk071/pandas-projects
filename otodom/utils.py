...
class Utils:
    ...
    @staticmethod
    def history_fill(
        main_df: pd.DataFrame, source_df: pd.DataFrame, fill_col: str, join_col: str
    ):
        main_df[fill_col].fillna(
            pd.merge(
                main_df, source_df, how="left", on=join_col, suffixes=("", "_new")
            )[fill_col + "_new"],
            inplace=True,
        )
...
class Geolocator:
    @staticmethod
    def geo_bing(adress: str, mode="dict"):
        bing = Bing(api_key=os.environ.get("BING_MAPS"))
        adress = adress.split(", ")
        if mode == "dict":
            query_dict = {
                "addressLine": adress[0],
                "locality": adress[-2],
    ...
    def geo_nominatim(adress: str, county=True):
        ...
        if county:
            query_dict = {
                "street": Utils.after_dot(adress[0]),
                "city": adress[-2],
                "county": adress[-3],
                "state": adress[-1],
                "country": "pl",
    ...
    @classmethod
    def get_geo(cls, adress):
        # try with county
        geo = cls.geo_nominatim(adress, county=True)
        if geo:
            return geo
        # try without county
        geo = cls.geo_nominatim(adress, county=False)
        if geo:
            return geo
        # try with bing and return to nominatim
        geo = cls.geo_bing(adress)
        if geo:
            la, lo, d = geo
            # print(1,d)
            ...
            if "postalCode" in d.keys():
                try:
                    geo = cls.geo_nominatim_postal(d["postalCode"])
                except:
                    try:
                        geo = cls.geo_nominatim_gps(la, lo)
                ...
                try:
                    geo = cls.geo_bing(adress, mode="short")
                    _, _, d = geo
                    # print(2,d)
                    geo = cls.geo_nominatim_postal(d["postalCode"])
                    # retain bing postal code
                    ...
                    try:
                        geo = cls.geo_bing(adress, mode="string")
                        la, lo, d = geo
                        geo = cls.geo_nominatim_gps(la, lo)
                        print("str")
                    except:
    ...
    @classmethod
    def add_geo(cls, series: pd.Series, source_col="where"):
        try:
            la, lo, d = cls.get_geo(series[source_col])
            ...
            (
                ...
                series["suburb"],
                series["postcode"],
            ) = (la, lo, d["neighbourhood"], d["suburb"], d["postcode"])
        except:
            print(series[source_col])
        return series

    @classmethod
    def geolocate_empty(cls, df, source_col="where"):
        # before geocoding
        # add missing columns
        for col in ["latitude", "longitude", "neighbourhood", "suburb", "postcode"]:
            Utils.add_empty_col(df, col)
        # prepare known list
        ...
        for col in ["latitude", "longitude", "neighbourhood", "suburb", "postcode"]:
            Utils.history_fill(df, known_without_dupli, col, source_col)
        ...
        new_without_dupli_gps = new_without_dupli.apply(
            lambda x: cls.add_geo(x, source_col), axis=1
        )
        # now fill based on new data
        for col in ["latitude", "longitude", "neighbourhood", "suburb", "postcode"]:
            Utils.history_fill(df, new_without_dupli_gps, col, source_col)
